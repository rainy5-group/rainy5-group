...
type ParseShadow = (penumbra: string) => Penumbra;

type GenerateShadow = (elevation: number, shadowColor?: string) => Shadow;

// ----------------------------------------------------------------------------
// Logic ----------------------------------------------------------------------
// ----------------------------------------------------------------------------

const parseShadow: ParseShadow = (penumbra) => {
...
const interpolate: Interpolate = (i, a, b, a2, b2) => {
 ...
 */
export const generateShadow: GenerateShadow = (
  elevation: number,
  shadowColor?: string
) => {
  let depth: number = elevation - 1;
  ...
  return {
    ...
    shadowRadius: Number.parseFloat(
      interpolate(shadow.blur, 1, 38, 1, 16).toFixed(2)
    ),
